...
export default function AppointmentDetailPage() {
    ...
    return (
        <main className="min-h-screen w-full bg-[#050505] max-w-md mx-auto flex flex-col font-sans relative border-x border-white/5 overflow-x-hidden">
            <div className="absolute inset-0 pointer-events-none">
                <div className="absolute top-0 right-0 w-64 h-64 bg-[#B87333]/8 blur-[100px] rounded-full" />
                <div className="absolute bottom-40 left-0 w-64 h-64 bg-[#B87333]/8 blur-[100px] rounded-full" />
            </div>
            ...
            <div className="px-6 pb-24 z-10 space-y-8">
                <motion.header initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }}>
                    <p className="text-[#D49A62]/70 text-[10px] font-black uppercase tracking-[0.3em] mb-1">Gerenciar</p>
                    <h2 className="text-4xl font-black text-[#F7EFE2] tracking-tighter leading-none">Agendamento</h2>
                </motion.header>
                ...
                <motion.button
                    ...
                    className={`w-full py-5 rounded-[2rem] font-black uppercase tracking-[0.2em] text-[12px] shadow-lg transition-all hover:scale-[0.98] active:scale-95 ${
                        isCompleted
                        ? "bg-white/[0.07] border border-white/10 text-[#F7EFE2]"
                        : "bg-[#B87333] text-[#050505] shadow-[#B87333]/25"
                    }`}
                >
                ...
                {!isCompleted && (
                    <motion.button
                        ...
                        className="w-full py-5 rounded-[2rem] border border-red-500/15 text-red-500/50 font-black uppercase tracking-[0.2em] text-[10px] transition-all hover:bg-red-500/5 active:scale-95"
                    >
                    ...
                    (activeSheet === "date" || activeSheet === "services") && (
                        <motion.button
                            ...
                            className={`w-full py-5 rounded-[2rem] font-black uppercase tracking-[0.2em] text-[11px] transition-all shadow-xl
                                ${isUpdating || (activeSheet === "services" && tempServiceIds.length === 0) ? 'bg-white/[0.05] text-[#F7EFE2]/25' : 'bg-[#B87333] text-[#050505] shadow-[#B87333]/20'}`}
                        >
            ...
            <div className="fixed bottom-0 left-0 right-0 h-20 bg-gradient-to-t from-[#050505] to-transparent pointer-events-none z-20" />
        </main>
    );
}