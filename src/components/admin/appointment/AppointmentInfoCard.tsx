...
export const AppointmentInfoCard = ({ 
  ...
  // Proteções contra dados vazios ou corrompidos (agendamentos antigos)
        ...
        <div className="flex flex-wrap gap-2.5">
          {safeServices.length > 0 ? (
            safeServices.map((s: any, index: number) => (
              <div key={s.id || index} className="bg-zinc-950 px-4 py-2.5 rounded-2xl text-zinc-200 font-black text-[11px] border border-zinc-800 uppercase tracking-tight">
                {s.name}
              </div>
            ))
          ) : (
            <p className="text-zinc-500 text-xs font-bold uppercase">Nenhum serviço (agendamento antigo)</p>
          )}
        </div>
      </div>
