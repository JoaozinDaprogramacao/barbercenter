...
export function CardPaymentActivity({ onBack, onClose }: { onBack: () => void, onClose: () => void }) {
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    // Função que chama nossa API assim que o componente é montado
    const initiateCardCheckout = async () => {
      try {
        const response = await fetch('/api/pagamento/assinatura', {
          ...
          body: JSON.stringify({
            // ATENÇÃO: Substitua pelos dados reais do usuário logado na sua plataforma!
            name: "João Emanuel", 
        ...
        if (data.checkoutUrl) {
          // Deu tudo certo! Redireciona para o Checkout Transparente do AbacatePay
